show_window_current_format() {
  local number="#I"
  local color=$(get_tmux_option "@dracuccin_window_current_color" "$thm_orange")
  local background=$(get_tmux_option "@dracuccin_window_current_background" "$thm_bg")
  local text="$(get_tmux_option "@dracuccin_window_current_text" "#{b:pane_current_path}")" # use #W for application instead of directory
